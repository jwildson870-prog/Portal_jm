...
def test_uploaded_image_is_served_with_image_mime(client, app):
    login(client, 'professor@portaljm.com', 'PortalJM@2026')
    with app.app_context():
        s = Series.query.first()
        sub = Subject.query.filter_by(series_id=s.id).first()
    r = client.post('/admin/contents/new', data={
        'title': 'Imagem', 'description': 'Teste de imagem',
        'series_id': str(s.id), 'subject_id': str(sub.id),
        'kind': 'file',
        'file': (io.BytesIO(b'fake-png-data'), 'foto.png')
    }, content_type='multipart/form-data')
    assert r.status_code == 302
    with app.app_context():
        c = Content.query.filter_by(title='Imagem').first()
        cid = c.id
        filename = c.file_name
    response = client.get(f'/aluno/arquivo/{cid}')
    assert response.status_code == 200
    assert response.mimetype == 'image/png'
    assert response.data == b'fake-png-data'
    assert os.path.exists(os.path.join(app.config['UPLOAD_FOLDER'], filename))


def test_uploaded_pdf_is_served_inline_with_pdf_mime(client, app):
    login(client, 'professor@portaljm.com', 'PortalJM@2026')
    with app.app_context():
        s = Series.query.first()
        sub = Subject.query.filter_by(series_id=s.id).first()
    r = client.post('/admin/contents/new', data={
        'title': 'PDF visual', 'description': '',
        'series_id': str(s.id), 'subject_id': str(sub.id),
        'kind': 'pdf',
        'file': (io.BytesIO(b'%PDF-1.7 test'), 'aula.pdf')
    }, content_type='multipart/form-data')
    assert r.status_code == 302
    with app.app_context():
        c = Content.query.filter_by(title='PDF visual').first()
        cid = c.id
    response = client.get(f'/aluno/pdf/{cid}')
    assert response.status_code == 200
