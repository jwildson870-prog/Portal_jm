...
def create_app(test_config=None):
    ...
    Path(app.root_path).parent.joinpath('uploads').mkdir(exist_ok=True)
    dburl = os.getenv('DATABASE_URL', 'sqlite:///portal_jm.db')
    if dburl.startswith('postgres://'):
        dburl = 'postgresql+psycopg2://' + dburl[11:]
    elif dburl.startswith('postgresql://'):
        dburl = 'postgresql+psycopg2://' + dburl[13:]
    app.config.update(
    ...
    def too_large(e):
        return (
            render_template('error.html', message='Arquivo muito grande. Limite: 25 MB.'),
            413)
    ...
    def create_admin():
        ...
        u, _ = ensure_admin()
        print('Administrador configurado.' if u else 'Defina ADMIN_EMAIL e ADMIN_PASSWORD.')

    @app.cli.command('seed')
    def seed():
        from .services import seed_initial_content
        seed_initial_content()
        print('Conteúdo inicial confirmado.')
