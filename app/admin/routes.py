...
def content_form(content=None):
    ...
    if request.method!='POST': return content,series,subjects
    sid=request.form.get('series_id',''); subid=request.form.get('subject_id',''); kind=request.form.get('kind','').strip(); title=request.form.get('title','').strip(); desc=request.form.get('description','').strip(); body=request.form.get('body','').strip(); external_url=request.form.get('external_url','').strip()
    s=Series.query.get(int(sid)) if sid.isdigit() else None; sub=Subject.query.get(int(subid)) if subid.isdigit() else None
    if not title or not s or not sub or sub.series_id!=s.id or kind not in ALLOWED_KINDS: flash('Preencha os dados obrigatórios corretamente.','error'); return None,series,subjects
    if kind in {'slide','video','link'} and not valid_url(external_url): flash('Informe um link HTTP/HTTPS válido.','error'); return None,series,subjects
    if kind=='explanation' and not body: flash('Escreva o conteúdo da explicação.','error'); return None,series,subjects
    old_file=content.file_name if content else None; new_file=old_file
    if kind in {'file','pdf'}:
        uploaded,original=save_uploaded_file(request.files.get('file') or request.files.get('pdf'),'pdf' if kind=='pdf' else None)
        if uploaded is False: flash('Para PDF envie .pdf.' if kind=='pdf' else 'Esse tipo de arquivo não é permitido.','error'); return None,series,subjects
...
def content_new():
    ...
    if content:
        notify_students(f'Novo material: {content.title}',url_for('student.content',id=content.id)); db.session.commit(); flash('Material publicado com sucesso.','success'); return redirect(url_for('admin.contents'))
...
def content_delete(id):
    content=Content.query.get_or_404(id); filename=content.file_name; db.session.delete(content); db.session.commit()
    if filename:
        try: os.remove(Path(current_app.config['UPLOAD_FOLDER'])/filename)
        except FileNotFoundError: pass
...
def user_delete(id):
    ...
    if user.id==current_user.id: flash('Você não pode excluir sua própria conta.','error')
    elif user.is_admin: flash('A conta de administrador não pode ser excluída por este painel.','error')
    else: db.session.delete(user); db.session.commit(); flash('Aluno excluído.','success')
...
def activity_edit(id):
    ...
    if request.method=='POST':
        ...
        if not title or not questions: flash('Informe o título e pelo menos uma questão.','error')
            ...
            or not all(isinstance(option, str) and option.strip() for option in q.get('options', []))
            or not isinstance(q.get('correct'), str)
            or q.get('correct') not in q.get('options', [])
            for q in questions
        ): flash('Cada questão precisa de enunciado, pelo menos duas alternativas e uma resposta correta.','error')
...
def activity_delete(id):
    a=Activity.query.get_or_404(id); db.session.delete(a); db.session.commit(); flash('Atividade excluída.','success'); return redirect(url_for('admin.activities'))

@admin_bp.route('/experiments',methods=['GET','POST'])
def experiments():
    if request.method=='POST':
        title=request.form.get('title','').strip(); sid=request.form.get('series_id',''); subid=request.form.get('subject_id',''); s=Series.query.get(int(sid)) if sid.isdigit() else None; sub=Subject.query.get(int(subid)) if subid.isdigit() else None
        if not title or not s or not sub or sub.series_id!=s.id: flash('Informe título, série e matéria.','error')
        else:
            e=Experiment(title=title,description=request.form.get('description','').strip(),objective=request.form.get('objective','').strip(),materials=request.form.get('materials','').strip(),steps=request.form.get('steps','').strip(),safety=request.form.get('safety','').strip(),conclusion=request.form.get('conclusion','').strip(),series_id=s.id,subject_id=sub.id); db.session.add(e); db.session.commit(); notify_students(f'Novo experimento: {e.title}',url_for('student.experiment',id=e.id)); db.session.commit(); flash('Experimento publicado.','success'); return redirect(url_for('admin.experiments'))
...
def experiment_edit(id):
    ...
    if request.method=='POST':
        sid=request.form.get('series_id',''); subid=request.form.get('subject_id',''); s=Series.query.get(int(sid)) if sid.isdigit() else None; sub=Subject.query.get(int(subid)) if subid.isdigit() else None
        if not request.form.get('title','').strip() or not s or not sub or sub.series_id!=s.id: flash('Dados inválidos.','error')
        else:
            for f in ['title','description','objective','materials','steps','safety','conclusion']: setattr(e,f,request.form.get(f,'').strip())
            e.series_id=s.id; e.subject_id=sub.id; db.session.commit(); flash('Experimento atualizado.','success'); return redirect(url_for('admin.experiments'))
