...
def index():
    return redirect(url_for('student.dashboard' if current_user.is_authenticated and (not current_user.is_admin) else 'admin.dashboard' if current_user.is_authenticated else 'home'))
...
def register():
    ...
    if request.method == 'POST':
        ...
        if not name or not email_ok(email) or (not p):
...
def google_login():
    if not os.getenv('GOOGLE_CLIENT_ID') or not os.getenv('GOOGLE_CLIENT_SECRET'):
        flash('Google OAuth não configurado no .env.', 'error')
        return redirect(url_for('auth.login'))
    return register_google().authorize_redirect(os.getenv(
        'GOOGLE_REDIRECT_URI',
...
def google_callback():
    try:
        token = register_google().authorize_access_token()
        info = token.get('userinfo') or register_google().userinfo()
    except Exception:
        flash('Falha na autenticação Google.', 'error')
        return redirect(url_for('auth.login'))
    sub = info.get('sub')
    email = (info.get('email') or '').lower().strip()
    if not sub or not email:
        flash('O Google não retornou dados suficientes.', 'error')
        return redirect(url_for('auth.login'))
